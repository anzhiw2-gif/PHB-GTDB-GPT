...
- P08 scaffold:
  ...
  - preparation accepts only approved P05 models, defaults to P06 `High-confidence`, permits `Review` only when explicit, preserves nonexclusive multi-model targets, and accepts only the P07-default canonical GTDB release `GTDB Release 11 R232`; it fail-closes on any alias/drift or if any P07 row differs from that release or normalized P06 candidate-table/scan-manifest sources, and also rejects P03 FAA-source mismatches, P07 duplicate status keys, family mismatch, or same-stem/different-path status association
  - each candidate retains P06 score/coverage thresholds, verified P03/P06/P07 source paths and whole-file SHA-256 contracts, plus actual InterProScan/SignalP6 terminal statuses and validated input/output paths; relative P03 manifest/QC `faa_path` declarations require an explicit `--p03-source-root`, retain both declared and resolved identities, and both resolved P03 paths must match P07; this is traceability only, not phenotype evidence
  ...
  - preparation accepts `--workers` only from 1 through 60 (default 1); it uses workers solely for independently preloading P07 candidate FASTA shards while the ordered validation/output writer remains single-threaded. Input SHA-256 values, including the dynamically located P05 core seed/close-control authority tables, are cached only while the normalized path's device, inode, size, and modification time remain unchanged; drift fails closed before artifact writing. Requested/effective preload workers are machine-local input provenance, not biological evidence
